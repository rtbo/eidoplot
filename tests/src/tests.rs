...
fn line_with_default() {
    ...
    assert_fig_eq_ref!(&fig, "line");
}

#[test]
fn line_with_plot_legend() {
    let x = vec![1.0, 2.0, 3.0];
    let y = x.clone();
...
fn line_with_y_minor_ticks() {
    ...
    assert_fig_eq_ref!(&fig, "line_y_minor_ticks");
}
