...
/// The representation is in seconds since Jan. 1, 2030, which is Eidoplot Epoch.
/// Timezone is no supported.
...
impl DateTime {
    ...
    /// Get the internal representation as a float
    ...
    pub const fn rep(&self) -> f64 {
        self.0
    }

    /// Build a new datetime from its internal representation
    ...
    pub const fn from_rep(rep: f64) -> Option<Self> {
        if rep.is_finite() {
            Some(DateTime(rep))
        } else {
            None
    ...
    /// Get this DateTime as a DateTimeComps
    pub fn as_comps(&self) -> DateTimeComps {
